...
- Every exit path closes the WebSocket and attempts idempotent HTTP release. A release isn't reported as
  successful without protocol evidence, and an unconfirmed release remains retryable only by the process
  holding its capability in memory. Ambiguous commands are not replayed.
