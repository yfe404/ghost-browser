...
- Every exit path with a Gateway-provided release capability closes the WebSocket and attempts idempotent
  HTTP release. A response missing both the session header and exact Ghost WebSocket path is rejected and
  reported as unconfirmed. Only exact-owner evidence can confirm release; an unconfirmed exact-owner release
  remains retryable by the process holding its capability in memory. Ambiguous commands are not replayed.
